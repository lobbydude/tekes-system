...
<script type="text/javascript">  
    ...
    // Select the value start
    function showjobpostdepartment(sel) {
        var j_id = sel.options[sel.selectedIndex].value;
        if (j_id.length > 0) {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('Recruitment/fetch_jobtitle') ?>",
                data: "j_id=" + j_id,
                cache: false,
                success: function (html) {
                    $("#add_skills_designation").html(html);
                }
            });
        }
    }
                    ...
                    <table class="table table-bordered datatable" id="jobpost_table">
                        ...
                        <tbody>
                            ...
                            foreach ($q->Result() as $row) {
                                $JP_Id = $row->JP_Id;                                
                                $Interview_date1 = $row->Interview_Date;                                
                                $Interview_date = date("d-m-Y", strtotime($Interview_date1));
