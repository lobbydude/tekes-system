...
foreach ($q_salary->result() as $row_salary) {
    ...
    $Monthly_CTC1 = $row_salary->Monthly_CTC;
    $Monthly_CTC = (round($Monthly_CTC1));
    ...
    $additional_insurance = $row_salary->Additional_Insurance; 
}
...
$Child_education = 0;
$Special_allowance = 0;
$Employer_PF_Amount1 = (($Basicpay + $Special_allowance) * 12) / 100;
$Employer_PF_Amount = (round($Employer_PF_Amount1));
...
$Employer_ESI = 0;
$Total_Fixed_Gross1 = $Monthly_CTC - ($Employer_ESI + $Employer_PF);
$Total_Fixed_Gross = (round($Total_Fixed_Gross1));

if ($Total_Fixed_Gross <= 21000) {
    $Employer_ESI1 = ($Total_Fixed_Gross * 4.75) / 100; 
    $Employer_ESI = (round($Employer_ESI1));
} else {
    $Employer_ESI = 0;
}
...
if ($Total_Fixed_Gross - ($Basicpay + $Hra + $Conveyance + $Medical) < 0) {
    ...
    $Skill_allowance1 = $Total_Fixed_Gross - ($Basicpay + $Hra + $Conveyance + $Medical); 
    $Skill_allowance = (round($Skill_allowance1));
}
if ($Total_Fixed_Gross <= 21000) {
    $Employer_ESI1 = ($Total_Fixed_Gross * 4.75) / 100;
    $Employer_ESI = (round($Employer_ESI1));
...
if ($Total_Fixed_Gross <= 21000) {
    $Employee_ESI1 = ($Total_Fixed_Gross * 1.75) / 100;
    $Employee_ESI = (round($Employee_ESI1));    
} else {
    $Employee_ESI = 0;
}
$Employee_PF_Amount1 = (($Basicpay + $Special_allowance) * 12) / 100;
$Employee_PF_Amount = (round($Employee_PF_Amount1));
...
// Additional Insurance start here
if($additional_insurance > 0){
    $additional_insurance = 200 + $additional_insurance;
    
}else{
    $additional_insurance = 0;
}
// Additional Insurance End here

$Net_Salary1 = $Total_Fixed_Gross - ($Employee_ESI + $Employee_PF + $Professional_Tax);
$Net_Salary = $Net_Salary1 - $additional_insurance ;

//$Net_Salary = $Total_Fixed_Gross - ($Employee_ESI + $Employee_PF + $Professional_Tax);
...
<h2 align='center' style='font-family:arial;'>Annexure 1.1</h2>
    <table border='1' style='border:1px solid #000'>
        ...
        <tr style='height:25px;font-family:arial;vertical-align:middle'>
            ...
            <td>" . number_format(round($additional_insurance), 2, '.', ',') . "</td>
            <td>" . number_format((round($additional_insurance) * 12), 2, '.', ',') . "</td>
        </tr>
        <tr style='height:25px;font-family:arial;vertical-align:middle'>
            <td>Income Tax</td>
            <td>0.00</td>
