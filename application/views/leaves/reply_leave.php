...
foreach ($q->result() as $row) {
    ...
    $Leave_From_view = date("d-m-Y", strtotime($Leave_From1));

    $Leave_To1 = $row->Leave_To;
    $Leave_To_include = date('Y-m-d', strtotime($Leave_To1 . "+1 days"));
    $Leave_To_view = date("d-m-Y", strtotime($Leave_To1));
    ...
    if ($Leave_Duration == "Full Day") {
        ...
        $No_days1 = $interval->format("%a") . " Days";
    } else {
        $total_no_days = 0.5;
        $No_days1 = "Half Day";
    }
    ...
    <div class="row">
        <div class="col-md-3">
                <div class="form-group">
                    ...
                    <select name="leave_reply_type" id="leave_reply_type" required value="<?php echo $Leave_Type; ?>" class="round" placeholder="Leave type" required data-validate="required" data-message-required="Please select Leave type">                                                
