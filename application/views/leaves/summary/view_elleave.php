...
$count = $q_leave_taken_el->num_rows();
?>
<script>
    function delete_elleave(id) {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Leaves/Deleteelleave') ?>",
            data: "leave_id=" + id,
            cache: false,
    ...
    <div class="row">
        <table class="table table-bordered datatable" id="leaves_table">
            ...
            <tbody>
                ...
                $i = 1;
                if ($count > 0) {
                    foreach ($q_leave_taken_el->Result() as $row) {
                        $Leave_Id = $row->L_Id;
                        ...
                        <tr>
							...
							<?php if ($user_role == 2 || $user_role == 6) { ?>
                            <td>
                                <a class="btn btn-danger btn-sm btn-icon icon-left" onclick="delete_elleave(<?php echo $Leave_Id; ?>)">
