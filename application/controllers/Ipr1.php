...
class Ipr1 extends CI_Controller {
    ...
    public function preview() {
        ...
        $this->load->view('ipr1/preview', $data);
		} else {
            redirect("Profile");
        }
    }
   ...
   // IPR Import Data Start here
    function import_ipr() {
        $import_type = $this->input->post('import_ipr_type');
        $filename = $_FILES["import_iprfile"]["tmp_name"];
        if ($import_type == "Effeciency_Statement") {
            if ($_FILES["import_iprfile"]["size"] > 0) {
                $add_ipr_import_year = $this->input->post('add_ipr_import_year');
                $add_ipr_import_month = $this->input->post('add_ipr_import_month');

                $this->db->truncate('tbl_effeciency');
                ...
                $count_no = 0;
                while (($effeciencyData = fgetcsv($file, 10000, ",")) !== FALSE) {
                    $count_no ++;
                    if ($count_no == 1) {
                        continue;
                    }
                    $insert_data = array(
                        'Year' => $add_ipr_import_year,
                        'Month' => $add_ipr_import_month,
                        'Username' => $effeciencyData[0],
                        'Employee_Id' => $effeciencyData[1],
                        'Production_Date' => $effeciencyData[2],
                        /* 'Client_Name' => $effeciencyData[3],
                          'Sub_Process' => $effeciencyData[4], */
                        'Order_Type' => $effeciencyData[3],
                        'Order_Type_ABS' => $effeciencyData[4],
                        'Order_Status' => $effeciencyData[5],
                        'State' => $effeciencyData[6],
                        'County' => $effeciencyData[7],
                        'Borrower_Name' => $effeciencyData[8],
                        'Progress_Status' => $effeciencyData[9],
                        'Start_Time' => $effeciencyData[10],
                        'End_Time' => $effeciencyData[11],
                        'Total_Time' => $effeciencyData[12],
                        'Order_Number' => $effeciencyData[13],
                        'Type' => $effeciencyData[14],
                        'Reporting_To' => $effeciencyData[15],
                        'Shift' => $effeciencyData[16],
                        'Task' => $effeciencyData[17],
                        'Target_Coding' => $effeciencyData[18],
                        'Category_Salary_Bracket' => $effeciencyData[19],
                        'Target' => $effeciencyData[20],
                        'Efficiency' => $effeciencyData[21],
                        'Inserted_By' => $inserted_id,
                        'Inserted_Date' => date('Y-m-d H:i:s'),
                        'Status' => 1
                    );
                    $this->db->insert('tbl_effeciency', $insert_data);
                }
                echo "Effeciency data Import Successfully";
            }
        } else if ($import_type == "Accuracy_Statement") {
            if ($_FILES["import_iprfile"]["size"] > 0) {
                $add_ipr_import_year = $this->input->post('add_ipr_import_year');
                $add_ipr_import_month = $this->input->post('add_ipr_import_month');

                $this->db->truncate('tbl_accuracy');
                $file = fopen($filename, "r");
                $sess_data = $this->session->all_userdata();
                $inserted_id = $sess_data['user_id'];

                $count_header2 = 0;
                while (($accuracyData = fgetcsv($file, 10000, ",")) !== FALSE) {
                    $count_header2++;
                    if ($count_header2 == 1) {
                        continue;
                    }
                    $insert_data2 = array(
                        'Year' => $add_ipr_import_year,
                        'Month' => $add_ipr_import_month,
                        'Employee_Id' => $accuracyData[0],
                        'Order_Number' => $accuracyData[1],
                        /* 'Client_Name' => $accuracyData[2],
                          'Subprocess_Name' => $accuracyData[3], */
                        'Order_Type' => $accuracyData[2],
                        'Error_Type' => $accuracyData[3],
                        'Error_Description' => $accuracyData[4],
                        'Comments' => $accuracyData[5],
                        'Error_Status' => $accuracyData[6],
                        'Error_Username' => $accuracyData[7],
                        'Error_User_DRN_Code' => $accuracyData[8],
                        'QC_Status' => $accuracyData[9],
                        'QC_Username' => $accuracyData[10],
                        'QC_User_DRN_Code' => $accuracyData[11],
                        'Production_Date' => $accuracyData[12],
                        'Analysis' => $accuracyData[13],
                        'Dispute_Comments' => $accuracyData[14],
                        'Error' => $accuracyData[15],
                        'Internal' => $accuracyData[16],
                        'External' => $accuracyData[17],
                        'Inserted_By' => $inserted_id,
                        'Inserted_Date' => date('Y-m-d H:i:s'),
                        'Status' => 1
                    );
                    $this->db->insert('tbl_accuracy', $insert_data2);
                }
                echo "Accuracy data Import Successfully";
            }
        } else if ($import_type == "Process_Knowledge_Statement") {
            if ($_FILES["import_iprfile"]["size"] > 0) {

                $add_ipr_import_year = $this->input->post('add_ipr_import_year');
                $add_ipr_import_month = $this->input->post('add_ipr_import_month');

                $this->db->truncate('tbl_process_knowledge');
                $file = fopen($filename, "r");
                $sess_data = $this->session->all_userdata();
                $inserted_id = $sess_data['user_id'];

                $count_header3 = 0;
                while (($process_knowledgeData = fgetcsv($file, 10000, ",")) !== FALSE) {
                    $count_header3++;
                    if ($count_header3 == 1) {
                        continue;
                    }
                    $insert_data3 = array(
                        'Year' => $add_ipr_import_year,
                        'Month' => $add_ipr_import_month,
                        'Employee_Id' => $process_knowledgeData[0],
                        'Employee_Name' => $process_knowledgeData[1],
                        'Rating' => $process_knowledgeData[2],
                        'Comments' => $process_knowledgeData[3],
                        'Inserted_By' => $inserted_id,
                        'Inserted_Date' => date('Y-m-d H:i:s'),
                        'Status' => 1
                    );
                    $this->db->insert('tbl_process_knowledge', $insert_data3);
                }
                echo "Process Knowledge data Import Successfully";
            }
        } else if ($import_type == "Teamwork_Statement") {
            if ($_FILES["import_iprfile"]["size"] > 0) {
                $add_ipr_import_year = $this->input->post('add_ipr_import_year');
                $add_ipr_import_month = $this->input->post('add_ipr_import_month');

                $this->db->truncate('tbl_teamwork');
                $file = fopen($filename, "r");
                $sess_data = $this->session->all_userdata();
                $inserted_id = $sess_data['user_id'];

                $count_header4 = 0;
                while (($teamworkData = fgetcsv($file, 10000, ",")) !== FALSE) {
                    $count_header4++;
                    if ($count_header4 == 1) {
                        continue;
                    }
                    $insert_data4 = array(
                        'Year' => $add_ipr_import_year,
                        'Month' => $add_ipr_import_month,
                        'Employee_Id' => $teamworkData[0],
                        'Employee_Name' => $teamworkData[1],
                        'Rating' => $teamworkData[2],
                        'Comments' => $teamworkData[3],
                        'Inserted_By' => $inserted_id,
                        'Inserted_Date' => date('Y-m-d H:i:s'),
                        'Status' => 1
                    );
                    $this->db->insert('tbl_teamwork', $insert_data4);

                    echo "Teamwork data Import Successfully";
                }
            }
        }
    }
    // IPR Import Data End here
	 ...
	 public function add_iprreport() { 
        $user_role = $this->session->userdata('user_role');
        if ($user_role == 2 || $user_role == 6) {
            $this->form_validation->set_rules('add_ipr_month', 'Month', 'trim|required');
            $this->form_validation->set_rules('add_ipr_year', 'Year', 'trim|required');
            //$this->form_validation->set_rules('add_ipr_empid', 'Emp Id', 'trim|required');
            $this->form_validation->set_rules('add_ipr_emp_name', 'Emp Name', 'trim|required');

            if ($this->form_validation->run() == TRUE) {                
                $add_ipr_month = $this->input->post('add_ipr_month');
                $add_ipr_year = $this->input->post('add_ipr_year');
                $add_ipr_empid = $this->input->post('add_ipr_empid'); 
                $add_ipr_empcode = $this->input->post('add_ipr_empcode'); 
                $add_ipr_emp_name = $this->input->post('add_ipr_emp_name');
                $add_ipr_emp_doj1 = $this->input->post('add_ipr_emp_doj');
                $add_ipr_emp_doj = date("Y-m-d", strtotime($add_ipr_emp_doj1));               
                $add_ipr_emp_destignation = $this->input->post('add_ipr_emp_destignation');
                $add_ipr_emp_reporting_person = $this->input->post('add_ipr_emp_reporting_person');
                
                $add_ipr_efficiency_rating = $this->input->post('add_ipr_efficiency_rating');
                $add_ipr_accuracy_rating = $this->input->post('add_ipr_accuracy_rating');
                $add_ipr_punctuality_rating = $this->input->post('add_ipr_punctuality_rating');
                $add_ipr_process_knowledge_rating = $this->input->post('add_ipr_process_knowledge_rating');
                $add_ipr_teamwork_rating = $this->input->post('add_ipr_teamwork_rating');
                $add_ipr_overallscore = $this->input->post('add_ipr_overallscore');
                
                $add_ipr_efficiency = $this->input->post('add_ipr_efficiency');
                $add_ipr_total_orders = $this->input->post('add_ipr_total_orders');
                $add_ipr_accuracy = $this->input->post('add_ipr_accuracy');
                $add_ipr_internal = $this->input->post('add_ipr_internal');
                $add_ipr_external = $this->input->post('add_ipr_external');
                $add_ipr_monthly_present = $this->input->post('add_ipr_monthly_present');
                $add_ipr_monthly_leave = $this->input->post('add_ipr_monthly_leave');
                $add_ipr_monthly_lop = $this->input->post('add_ipr_monthly_lop');
                $add_ipr_monthly_weekend_login = $this->input->post('add_ipr_monthly_weekend_login');
                $add_ipr_final_rating = $this->input->post('add_ipr_final_rating');
                $add_ipr_efficiency_comments = $this->input->post('add_ipr_efficiency_comments');
                $add_ipr_accuracy_comments = $this->input->post('add_ipr_accuracy_comments');
                $add_ipr_punctuality_comments = $this->input->post('add_ipr_punctuality_comments');
                $add_ipr_process_knowledge_comments = $this->input->post('add_ipr_process_knowledge_comments');
                $add_ipr_teamwork_comments = $this->input->post('add_ipr_teamwork_comments');
                
                $add_ipr_supervisor_comments = $this->input->post('add_ipr_supervisor_comments');
                $add_ipr_employee_comments = $this->input->post('add_ipr_employee_comments');
                $add_ipr_emp_other_comments = $this->input->post('add_ipr_emp_other_comments');
                $add_ipr_sup_other_comments = $this->input->post('add_ipr_sup_other_comments');
                $add_ipr_supervisor_date1 = $this->input->post('$add_ipr_supervisor_date1');
                $add_ipr_supervisor_date = date("Y-m-d", strtotime($add_ipr_supervisor_date1));           
                $add_ipr_employee_date1 = $this->input->post('add_ipr_employee_date');
                $add_ipr_employee_date = date("Y-m-d", strtotime($add_ipr_employee_date1));
                $add_ipr_supervisor_signature = $this->input->post('add_ipr_supervisor_signature');
                $add_ipr_employee_signature= $this->input->post('add_ipr_employee_signature');                              
                ...
                $insert_data = array(
                    'Month' => $add_ipr_month,
                    'Year' => $add_ipr_year,
                    'Employee_Id' => $add_ipr_empid,
                    'Emp_Code' => $add_ipr_empcode,
                    'Employee_Name' => $add_ipr_emp_name,
                    'DOJ' => $add_ipr_emp_doj,
                    'Designation' => $add_ipr_emp_destignation,
                    'Reporting_Person' => $add_ipr_emp_reporting_person,
                    'Efficiency_Rating' => $add_ipr_efficiency_rating,
                    'Accuracy_Rating' => $add_ipr_accuracy_rating,
                    'Punctuality_Rating' => $add_ipr_punctuality_rating,
                    'Process_Knowledge_Rating' => $add_ipr_process_knowledge_rating,
                    'Teamwork_Rating' => $add_ipr_teamwork_rating,
                    'Overall_Score' => $add_ipr_overallscore,
                    
                    'Efficiency' => $add_ipr_efficiency,
                    'Total_Orders' => $add_ipr_total_orders,
                    'Accuracy' => $add_ipr_accuracy,
                    'Internal' => $add_ipr_internal,
                    'External' => $add_ipr_external,
                    'Monthly_Present' => $add_ipr_monthly_present,
                    'Monthly_Leave' => $add_ipr_monthly_leave,
                    'Monthly_LOP' => $add_ipr_monthly_lop,                    
                    'Monthly_Weekend_Login' => $add_ipr_monthly_weekend_login,
                    'Final_Rating' => $add_ipr_final_rating,
                    'Efficiency_Comments' => $add_ipr_efficiency_comments,
                    'Accuracy_Comments' => $add_ipr_accuracy_comments,
                    'Punctuality_Comments' => $add_ipr_punctuality_comments,
                    'Process_Knowledge_Comments' => $add_ipr_process_knowledge_comments,
                    'Teamwork_Comments' => $add_ipr_teamwork_comments,
                    'Supervisor_Comments' => $add_ipr_supervisor_comments,
                    'Employee_Comments' => $add_ipr_employee_comments,
                    'Employee_Other_Comments' => $add_ipr_emp_other_comments,
                    'Supervisor_Other_Comments' => $add_ipr_sup_other_comments,
                    
                    'Supervisor_Date' => $add_ipr_supervisor_date,
                    'Employee_Date' => $add_ipr_employee_date,
                    'Supervisor_Signature' => $add_ipr_supervisor_signature,
                    'Employee_Signature' => $add_ipr_employee_signature,                   
                    
                    'Inserted_By' => $inserted_id,
                    'Inserted_Date' => date('Y-m-d H:i:s'),
                    'Status' => 1
                );
                $q = $this->db->insert('tbl_iprreport', $insert_data);
                echo "IPR data Successfully Inserted";
                if ($q) {
                    echo "success";
                } else {
                    echo "fail";
	...
	/* IPR Employee Preview start here*/
    public function Employee_IPR() {
        $data = array(
            'title' => 'Employee IPR-Report',
            'main_content' => 'ipr1/ipremployee'
        );
        $this->load->view('Common/content', $data);
    }
    public function Emp_Iprpreview() {
        $employee_list = $this->input->post('employee_list');
    ...
    /* IPR Employee Preview End here*/
    
    /* IPR Dashboard Preview Start here*/   
    public function Iprdashboard() {
        $user_role = $this->session->userdata('user_role');
        if ($user_role == 2 || $user_role == 6 || $user_role == 1) {
            $data = array(
                'title' => 'IPR Dashboard',
                'main_content' => 'ipr1/iprdashboard'
            );
            $this->load->view('operation/content', $data);
        } else {
            redirect('Profile');
    ...
    /* IPR Dashboard Preview End here*/
